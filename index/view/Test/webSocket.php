...
//下面是sock类
class webSocket{
    public $sockets; //socket的连接池，即client连接进来socket_create的socket标志
    ...
    function run(){
        ...
        while(true){
            ...
            */
            ...
            echo iconv('utf-8','gbk//IGNORE','Total：'.count($this->users)."\n");

            foreach($changes as $sock){
                //如果有新的client连接进来，则
                if($sock==$this->master){
                    echo iconv('utf-8','gbk//IGNORE','Step1：'.count($this->users)."\n");
                    ...
                    echo iconv('utf-8','gbk//IGNORE','Step2：'.count($this->users)."\n");
    ...
    */
    ...
    function send1($k,$ar,$key='all'){
        ...
        if($key=='all'){
            ...
            $str = $this->code(json_encode($ar));
            //单独对个人发送信息，即双方聊天
//            socket_write($this->users[$k]['socket'],'对方已接收',5);
